...
Because our brain limits the complexity that we can manage at any single point in time¹, it is better to carve up a large task into a number of smaller tasks, each doing one thing and doing it well. We can then focus on the small tasks, one at a time: deal with one and move on to the next. This mental strategy of "divide and conquer²" is often referred to as [abstraction](https://en.wikipedia.org/wiki/Abstraction_(software_engineering)).
...
## Example: Eating lunch (tomato soup)
...
| `warmUp(food)` | Warns up the food and return the heated food. |
...
We will develop five versions ot this example. The first version is the most simple, without any asynchronous activity. In versions two and beyond we will introduce the factor of time.

### Version 1: Base version

File: [1-functions-sync.js](1-functions-sync.js)

A complete solution using regular functions is shown below in Listing 1.
...
For the examples we will use accelerated time, otherwise we will just be waiting too long.
...
We will also use two helper functions, notably `startTimer()` and `stopTimer()`, to show the elapsed time in seconds while we are waiting. These helper functions resides in a separate file (`timer.js`), so that they do not clutter up the code we want to focus on. To access it, we must some syntax from Node (see below).  If you are unfamiliar with Node just take it for granted for now that this syntax makes the functions `startTimer()` and `stopTimer()` available for use in our code. 

```js
const { startTimer, stopTimer } = require('./timer');
```

As our tasks take time now, we can no longer use the `return` statement to return the result of a task. We need to rewrite our functions to handle the asynchronous nature of the tasks. In version 2 of the example as described in this secion we will use _callback_ functions to achieve this.

| Function | Description |
| -------- | ----------- |
| `openCan(foodCan, onOpened)` | Opens a food can and when done, calls the `onOpened` callback passing it the opened contents. |
| `warmUp(food, onReady)` | Warns up the food and when done, calls the `onReady` callback return the heated food. |
| `eat(food, onReady)` | Writes to the console that we are eating the food and when done, calls the `onReady` callback. |
| `eatLunch(cannedFood, onReady)` | Takes a can of food as its argument and calls the previous three functions to prepare and eat the canned food. It call the `onReady` callback when done |
| `main()` | Starts the timer, calls `eachLunch()` passing it the `campbellsTomatoSoup` object and stops the timer when the lunch has finished. |  
...
The revised `openCan()` function now uses the standard `setTimeout()` function to simulate the time it takes to open a can (2 secs). When the time has passed, it call the `openOpened` function, passed as argument by its caller, and passes it the opened contents of the food can.
...
Because we cannot warm the contents of the can before it has been opened, we can must handle warming up of the content inside the callback we passed to `openCan()`.  

Similarly, we cannot eat the hot food until it has been fully warmed up. Therefore, we need to receive the hot food by passing a callback to `warmUp()` that will receive the hot food (when ready) in its argument `hotFood`. 

We can now start eating the hot food. When done we need to call the `onReady` callback give notification that we have finished out lunch.
...
Listing 3. The `main()` function, asynchronous version (`oconsole)
...
When we finally call our `main()` function, the following output will be produced (taking 10 seconds in total to complete):
...
To convert a regular anonymous function to a fat arrow function you can just take out the `function` keyword and place a "fat arrow" `=>` between the closing parenthesis of the argument list and the opening curly brace of the function body.
...
If the function body of a fat arrow function consists of one line only, the function can be written even more concise by taking our the curly braces too. If the function returns a value, the `return` keyword must be taken out too. For example, the following fat arrow function return the square of a number:
...
We will not repeat here the explanation that we have provided in the [fundamental of promises](https://github.com/HackYourFuture/fundamentals/blob/master/fundamentals/promises.md). But let's look how can convert the `openCan()` function.

We no longer need a callback parameter in our `openCan()` function, only the `foodCan` that we want to open. In the promise we create a promise wit `new Promise(resolve => { ... })`. Not that, because we are not rejecting anything, we don't need a `reject` argument in the function that we pass to the `Promise` constructor. Our `openCan()` function return the newly created promise.
...
Our `eatLunch()` and `main()` functions can now be rewritten as follows (`console.log` statements taken out of brevity):
...
In the final version, we will "consume" the promises with `async` and `await`, as described in the [fundamental on async/await](https://github.com/HackYourFuture/fundamentals/blob/master/fundamentals/async_await.md). This code looks like where the factor time has been taken out again. Note however, that we still need to create promises to begin with. It's only on the "consuming" side that we can use `async` and `await`.
